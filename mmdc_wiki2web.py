...
import html5lib, urllib.request, urllib.parse, urllib.error, pprint, re
...
def create_pages(memberpages, mode):
    ...
    for member in memberpages:
        ...
        if len(articledict['Date'])>0 and len(articledict['Creator'])>0 and len(articledict['Title'])>0  and len(articledict['Thumbnail'])>0:
            for key in list(articledict.keys()):
                print(key)
...
#                    print 'FOUND:', found
        ...
        page_title_date.text = "{} {}".format(articledict['Title'], articledict['Date'])
        page_description = page_tree.find('.//div[@id="description"]')
        description_html = articledict['Description']
        page_description.text = None  # Clear existing text content
        if(description_html):
           page_description.append(ET.fromstring('<div>'+description_html+'</div>'))
        page_bio = page_tree.find('.//div[@id="bio"]')
        page_bio.text = None  # Clear existing text content
        page_bio.append(ET.fromstring('<div>'+articledict['Bio']+'</div>'))
        ...
        page_extra.text = None  # Clear existing text content
        page_extra.append(ET.fromstring('<div>'+articledict['Extra']+'</div>'))
        ...
        if len(figures) > 0:
            for figure in figures:
                ...
                src = (('File:'+img_src).capitalize())
                if src in list(articledict['Imgs'].keys()): #full-url
...
if user_args.preview:
    memberpages=[str(user_args.preview)]
else:    
    ...
    for key in list(indexdict.keys()): # populate indexdict_byyear with works
        indexdict_byyear[ int(indexdict[key]['Date'])][key] = indexdict[key]

    print('\n\n******* indexdict_byyear ***********\n\n')
    pprint.pprint( indexdict_byyear )    

    for year in list(indexdict_byyear.keys()): # create index page for each year ie 2016.html
        print('\n***** ', year, ' *****\n')
